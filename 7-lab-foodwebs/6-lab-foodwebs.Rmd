...
probEat <- function(alpha, beta, gamma, inv_logit, m1, m2) {
  return (inv_logit(alpha + (beta * log(m2 / m1)) + (gamma * log(m2 / m1) ** 2)))
}
...
```{r create-adj-mat}
feeding_prob <- rep(0, 1)
feeding_prob
for(row in sp_mass) {
  probEat(alpha, beta, gamma, inv_logit, sp_mass$f. sp_mass$m)
}

probEat(alpha, beta, gamma, inv_logit, 10, 2)
