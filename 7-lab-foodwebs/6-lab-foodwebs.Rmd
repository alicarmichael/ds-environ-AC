...
probEat <- function(alpha, beta, gamma, inv_logit, m1, m2) {
  return (round((inv_logit(alpha + (beta * log(m2 / m1)) + (gamma * log(m2 / m1)^2))), digits = 2))
}
```
...
```{r create-adj-mat}
num_spec <- nrow(sp_mass)
feeding_matrix <- matrix(0, nrow = num_spec, ncol = num_spec,
                     dimnames = list(rownames(sp_mass), rownames(sp_mass)))

for (i in 1:num_spec) {
  for (j in 1:num_spec) {
    pred_mass <- sp_mass[i, "f"]
    prey_mass <- sp_mass[j, "f"]
    feeding_matrix[i, j] <- probEat(alpha, beta, gamma, inv_logit, pred_mass, prey_mass)
  }
}
...
```{r create-adj-mat-2}
feeding_matrix <- sweep(feeding_matrix, 1, carnivores, "*")
feeding_matrix <- sweep(feeding_matrix, 2, carnivores, "*")

print(rowSums(feeding_matrix))
...
```{r t1-time-webs}
spec_in_p2 <- sp_occ[, 2] == 1
print(sum(spec_in_p2))

new_feeding_matrix <- feeding_matrix[spec_in_p2, spec_in_p2]

new_feeding_matrix_clean <- sweep(new_feeding_matrix, 1, spec_in_p2[spec_in_p2], "*")
new_feeding_matrix_clean <- sweep(new_feeding_matrix_clean, 2, spec_in_p2[spec_in_p2], "*") 

print(dim(new_feeding_matrix_clean))
cat("Number of non-zero interactions after cleaning:\n")
print(sum(new_feeding_matrix_clean > 0))
print(new_feeding_matrix_clean)
## Assuming sp_occ is a matrix or data frame with species occurrence across time periods
# and feeding_matrix is the interaction matrix of species
?sweep
```
